...
func _connect_signals() -> void:
	...
	combat.parry_succeeded.connect(_on_parry_succeeded)
...
func _physics_process(delta: float) -> void:
	...
	var can_move: bool = state_machine.can_attack()
...
func _update_state() -> void:
	# Movement states only - combat overlays and doesn't block movement
...
func _update_visuals() -> void:
	...
	# Movement sprite - always based on movement state
	var sprite_key: String
	var anim_name: String
	
	if state_machine.current_state == PlayerStateMachineScript.State.DASH:
		sprite_key = "dash"
		anim_name = "Dash"
	elif not is_on_floor():
		sprite_key = "jump"
	...
	# Combat overlay sprite - shown on top of movement during attack
	if combat.is_attacking():
		visuals.show_combat_sprite("attack")
	else:
		visuals.hide_combat_sprite()
		# Only play movement animation when not attacking
...
func _on_attack_started() -> void:
	visuals.play_combat_animation("combat_attack", 2.0)
...
func _on_parry_succeeded(_projectile: Node) -> void:
	visuals.trigger_particles("parry", movement.facing_direction)


func _on_animation_finished(anim_name: String) -> void:
	if anim_name in ["combat_attack", "Attack", "attack"]:
